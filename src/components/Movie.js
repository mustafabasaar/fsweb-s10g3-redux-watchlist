  ...
  return (
    <div className="flex bg-white shadow-lg items-start">
      ...
      <div className="p-8 flex flex-col gap-4 text-sm">
        <div>
          <h2 className="text-2xl">{props.movie.title}</h2>
          <p className="italic text-sm">{props.movie.genres.join(", ")}</p>
        </div>
        <p className="">{props.movie.plot}</p>
        <div className="flex flex-col sm:flex-row">
          <span className="w-1/3 font-bold">Yönetmen</span>
          <span className="flex-1">{props.movie.director}</span>
        </div>
        <div className="flex flex-col sm:flex-row">
          <span className="w-1/3 font-bold">Oyuncular</span>
          <span className="flex-1">{props.movie.actors}</span>
        </div>
